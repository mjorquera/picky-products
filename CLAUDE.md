...
2. Pin image quality — templates may not have been strong enough to earn clicks/saves.
3. Pinterest itself — may not be a high-converting channel for this kind of affiliate offer at this account's scale/authority.

**Reactivation:** revisit if there's a reason to believe one of the above hypotheses can be tested cheaply, or if circumstances change (e.g. Associates reinstated some other way, new data suggests a fix). Until then, treat this as archived rather than in-flight.
...
**Resolved (2026-07-16):** the Make scenario's Pinterest module routes `board_id` dynamically per record (`{{5.board_id}}`, fixed 22 Jun) and `.env`/`board_id_for_pin()` are both correct. Confirmed live: Cosi Home's `pin-1-hot-clean.png` published 2026-07-16 and landed on `PINTEREST_BOARD_HOT` as expected. WAL-110 can be closed.
