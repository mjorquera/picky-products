...
**Resolved (2026-07-15):** the Make scenario's Pinterest module already routes `board_id` dynamically per record (`{{5.board_id}}`, fixed 22 Jun) and `.env`/`board_id_for_pin()` are both correct — verified via Make execution history and a live API check. `PINTEREST_BOARD_HOT` still shows 0 pins only because every Hot Sleeper pin published so far predates the 22 Jun fix; no Hot pin has published since. WAL-110 stays open until the next Hot pin (Cosi Home, due 2026-07-16) confirms it lands on the right board.
